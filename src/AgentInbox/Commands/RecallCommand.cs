...
public static class RecallCommand
{
    public static Command Build(Option<string> dbPathOption, Option<OutputFormat> formatOption)
    {
        ...
        var cmd = new Command(CommandNames.Recall, CommandNames.Descriptions.Recall)
        {
            tokenOpt,
            queryOpt,
            modeOpt,
        ...
        cmd.SetAction((ParseResult parseResult) =>
        {
            ...
            try
            {
                ...
                if (mode == "semantic")
                    return RunSemanticRecall(ctx, agentId, query, embeddingJson, limit, formatter);

                if (string.IsNullOrWhiteSpace(query))
                    return CommandExecution.Fail(formatter, CommandNames.Messages.SearchQueryRequired);

                return RunTextRecall(ctx, agentId, query, limit, formatter);
            }
            catch (Exception ex)
            {
    ...
    private static int RunTextRecall(DbContext ctx, string agentId, string query, int limit, IOutputFormatter formatter)
    {
        var conn = ctx.Connection;
    ...
    private static int RunSemanticRecall(DbContext ctx, string agentId, string? query, string? embeddingJson, int limit, IOutputFormatter formatter)
    {
        if (!ctx.VecLoaded)
        ...
        {
            if (!SearchCommand.TryParseEmbedding(embeddingJson, formatter, out queryEmbedding))
                return 1;
        }
        ...
        searchCmd.CommandText = """
            ...
            """;
        searchCmd.Parameters.AddWithValue("@queryEmbedding", SearchCommand.SerializeEmbeddingForVec(queryEmbedding));
        searchCmd.Parameters.AddWithValue("@limit", limit);
        searchCmd.Parameters.AddWithValue("@agentId", agentId);
    ...
    private static List<SearchResult> ReadSearchResults(SqliteCommand cmd)
    {
